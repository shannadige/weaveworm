...
`design/`, `deliver/`) live under the working directory: read and write
them by those relative paths, and never build an absolute path from
where the spec lives, because `${CLAUDE_PLUGIN_ROOT}` and the folder
above it hold only spec and voice files, are read-only, and are never
listed, searched, or written. Conversation runs per the voice contract
