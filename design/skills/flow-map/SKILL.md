...
can walk. Input: a brief whose Status carries a concept-sprint pick
(`in-design (→ D-NNN)`), the chosen `C-NNN`, and the journey moment
the brief cites. No pick on record means nothing to articulate —
route to concept-sprint; a flow drawn from an unchosen concept
launders the pick. Output: `design/flows/F-NNN-<slug>.md` per the
...
- **Material change supersedes; it never edits in place.** Re-pointing
  ...
  a fresh `F-NNN` citing the new decision, the old file marked
  superseded with the pointer (mirroring decision reversal — the old
  record stays). Then sweep: prototype screens whose `data-flow`
...
prototyped first, and which edge is the riskiest one still untested —
name both with the one-line why (usually the edge whose failure costs
the brief's `Done when:`). When a stage sits at `(needs one)` or an
